...
describe('buildToolResult', () => {
  ...
  test('fanout tool cards show invocation count and execution mode', () => {
    const parallel = buildToolCard({
      id: 'fanout-parallel',
      name: 'bash',
      args: {},
      status: 'running',
    ...
    expect(parallel[0]!.text).toContain('bash ×3 parallel')

    const sequential = buildToolCard({
    ...
    expect(sequential[0]!.text).toContain('read ×2 sequential')
  })

  test('expanded fanout results put each command before its output', () => {
    const lines = buildToolCard({
      id: 'fanout-commands',
      name: 'bash',
      args: {},
      ...
      details: {
        fanout: {
          count: 2,
          children: [
            { index: 0, preview_command: 'echo first' },
            { index: 1, preview_command: `printf "second
line"` },
          ],
        },
      },
    ...
    expect(text.indexOf('$ echo first')).toBeLessThan(text.indexOf('first output'))
    expect(text.indexOf('$ printf "second')).toBeLessThan(text.indexOf('second output'))
    expect(text).toContain('  line"')
  })

  test('collapsed fanout results do not duplicate commands', () => {
    const lines = buildToolCard({
      id: 'fanout-collapsed',
      name: 'bash',
      args: {},
      ...
      details: {
        fanout: {
          children: [{ index: 0, preview_command: 'echo first' }],
        },
      },
    })
